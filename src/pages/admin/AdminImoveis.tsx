...
import { useImoveisAdmin, useUpdateImovel, useDeleteImovel } from "@/hooks/useImoveis";
...
export default function AdminImoveis() {
  ...
  const { data: properties = [], isLoading } = useImoveisAdmin({ search, status: filterStatus, finalidade: filterFinalidade });
  const updateMutation = useUpdateImovel();
  const deleteMutation = useDeleteImovel();

  const handleDelete = (id: string) => {
    if (window.confirm("Tem certeza que deseja excluir este imóvel?")) {
      deleteMutation.mutate(id, { onSuccess: () => toast.success("Imóvel excluído.") });
    }
  };

  const toggleStatus = (id: string, current: string) => {
    updateMutation.mutate(
      { id, status: current === "ativo" ? "vendido" : "ativo" },
      { onSuccess: () => toast.success("Status atualizado.") }
    );
  };

  const toggleDestaque = (id: string, current: boolean | null) => {
    updateMutation.mutate(
      { id, destaque: !current },
      { onSuccess: () => toast.success("Destaque atualizado.") }
    );
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <Button onClick={() => navigate("/admin/imoveis/novo")} className="bg-[hsl(var(--nova-orange))] hover:bg-[hsl(var(--nova-orange))]/90 text-white">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <Input placeholder="Buscar por título ou código..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-9" />
        </div>
        ...
        <Select value={filterFinalidade} onValueChange={setFilterFinalidade}>
          <SelectTrigger className="w-full sm:w-40"><SelectValue placeholder="Finalidade" /></SelectTrigger>
          <SelectContent>
            <SelectItem value="todos">Todos</SelectItem>
        ...
        <Table>
          ...
          <TableBody>
            {isLoading ? (
              ...
              <TableRow><TableCell colSpan={9} className="text-center py-8 text-muted-foreground">Nenhum imóvel encontrado.</TableCell></TableRow>
            ) : (
              properties.map((p) => (
                <TableRow key={p.id}>
                  ...
                  <TableCell className="hidden sm:table-cell">
                    <button onClick={() => toggleDestaque(p.id, p.destaque)}>
                      {p.destaque ? <Star className="w-4 h-4 text-[hsl(var(--nova-orange))] fill-current" /> : <StarOff className="w-4 h-4 text-muted-foreground" />}
                    </button>
                  </TableCell>
                  <TableCell>
                    <div className="flex items-center justify-end gap-1">
                      <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => navigate(`/admin/imoveis/editar/${p.id}`)}><Pencil className="w-4 h-4" /></Button>
                      ...
                      <Button variant="ghost" size="sm" className="h-8 text-xs hidden lg:flex" onClick={() => toggleStatus(p.id, p.status || "ativo")}>
