...
export default function HomePage() {
  ...
  const destaque = imoveis.filter((i) => i.destaque && i.status === "ativo").slice(0, 6);
  const tabImoveis = imoveis.filter((i) => {
    if (activeTab === "venda") return i.finalidade === "venda" && i.tipo !== "terreno" && i.tipo !== "comercial" && i.status === "ativo";
    if (activeTab === "aluguel") return i.finalidade === "aluguel" && i.status === "ativo";
    if (activeTab === "terrenos") return i.tipo === "terreno" && i.status === "ativo";
    if (activeTab === "comercial") return i.tipo === "comercial" && i.status === "ativo";
          ...
          <div className="animate-reveal-up max-w-3xl mx-auto bg-white rounded-2xl p-2 shadow-2xl" style={{ animationDelay: "200ms" }}>
            <div className="flex flex-col md:flex-row gap-2">
              <select
                ...
                className="h-12 rounded-xl border-0 bg-muted px-4 text-sm font-medium text-foreground focus:ring-2 focus:ring-primary"
              >
              ...
              <select
                ...
                className="h-12 rounded-xl border-0 bg-muted px-4 text-sm text-foreground focus:ring-2 focus:ring-primary"
              >
              ...
              <select
                ...
                className="h-12 rounded-xl border-0 bg-muted px-4 text-sm text-foreground flex-1 focus:ring-2 focus:ring-primary"
              >
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
              {categories.map((cat) => (
                <Link
                  key={cat.label}
                  to={cat.link}
                  className="group flex flex-col items-center text-center p-6 rounded-xl border border-border/50 hover:shadow-lg hover:border-primary/20 transition-all duration-300"
                >
          ...
          <ScrollReveal>
            <div className="text-center mb-12">
              ...
              <p className="text-muted-foreground mt-2 max-w-xl mx-auto">
                Mais do que uma imobiliária: segurança jurídica e transparência em cada negociação
              </p>
            </div>
          </ScrollReveal>
        ...
        <div className="container text-center">
          <ScrollReveal>
            ...
            <p className="text-white/70 text-lg max-w-xl mx-auto mb-8">
              Anuncie com quem oferece segurança jurídica. Seu imóvel merece uma gestão profissional.
            </p>
